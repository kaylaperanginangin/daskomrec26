...
export default function ChangePassword({ users = { data: [] } }) {
    ...
    const getFormStyle = () => ({
        ...
        transform: !inputLocked && !isLoggingOut && !isExiting ? "translateY(0)" : "translateY(20px)",
        ...
        <>
            ...
            <div className="relative w-full h-screen overflow-hidden text-white font-caudex">
                ...
                <div className="absolute inset-0 z-10 pointer-events-none overflow-hidden">
                    {bubbles.map((b, i) => (
                        <div key={i} className="bubble" style={{ left: b.left, width: b.size, height: b.size, animation: `rise ${b.duration} linear ${b.delay} infinite`, opacity: b.opacity }} />
                    ))}
                </div>

                {/* Road */}
                <div className="absolute inset-0 z-10 pointer-events-none">
                    <img src={ImgRoad} alt="ImgRoad" className="absolute bottom-0 left-0 w-full h-auto object-cover" style={{ filter: "sepia(1) hue-rotate(150deg) saturate(2) contrast(1.5) brightness(0.9)" }} />
                </div>
                ...
                <div className="absolute inset-0 z-20 flex flex-col items-center justify-center" style={getFormStyle()}>
                    ...
                    <div className="w-full max-w-[450px] px-8 space-y-6">
                        ...
                        <div className="group">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full h-12 bg-gray-300 text-gray-800 rounded-md px-4 pr-12 shadow-inner focus:outline-none focus:ring-2 focus:ring-blue-400 focus:bg-white transition-all placeholder-gray-500"
                                />
                                <button type="button" onClick={() => setShowNewPassword(!showNewPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 hover:text-gray-900 transition-colors focus:outline-none">
                        ...
                        <div className="group">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full h-12 bg-gray-300 text-gray-800 rounded-md px-4 pr-12 shadow-inner focus:outline-none focus:ring-2 focus:ring-blue-400 focus:bg-white transition-all placeholder-gray-500"
                                />
                                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 hover:text-gray-900 transition-colors focus:outline-none">
                        ...
                        <div className="flex justify-center mt-8">
                            <button onClick={handleChangeClick} className="relative w-64 h-16 flex items-center justify-center group active:scale-95 transition-transform">
                                <img src={ButtonRegular} alt="change button" className="absolute inset-0 w-full h-full object-fill pointer-events-none drop-shadow-lg" style={{ filter: "brightness(0.9) hue-rotate(10deg) saturate(1.2)" }} />
                                <span className="relative z-10 text-white text-3xl font-caudex tracking-widest pb-1 drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]">Change</span>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <BlueModalWrapper isOpen={showModal} onClose={() => setShowModal(false)}>
                    <div className="flex flex-col justify-center items-center text-center h-full w-full space-y-3 transition-all modal-anim">
                        <h1 className="font-caudex text-xs text-white px-10 sm:px-0 tracking-[0.2em] uppercase drop-shadow-[0_2px_3px_rgba(0,0,0,0.8)] leading-tight font-bold">Let The Deep Uncover Your Purpose</h1>
                        <h1 className="font-caudex text-xl sm:text-4xl text-white drop-shadow-[0_2px_3px_rgba(0,0,0,0.8)] leading-tight font-bold">Are you sure to <br /> change the password?</h1>
                        <div className="flex flex-col sm:flex-row gap-2 sm:gap-6 mt-4">
                            <button onClick={() => setShowModal(false)} className="relative flex items-center justify-center h-12 min-w-[150px] md:min-w-[180px] px-6 group active:scale-95 transition-transform">
                                ...
                                <span className="relative z-10 text-white text-2xl">No</span>
                            </button>
                            <button onClick={handleConfirmChange} className="relative flex items-center justify-center h-12 min-w-[150px] md:min-w-[180px] px-6 group active:scale-95 transition-transform">
                                ...
                                <span className="relative z-10 text-white text-2xl">Yes</span>
                            </button>
                        </div>
                    </div>
                </BlueModalWrapper>
                ...
                <BlueModalWrapper isOpen={showSuccess} onClose={() => setShowSuccess(false)}>
                    <div className="flex flex-col justify-center items-center text-center h-full w-full space-y-3 transition-all modal-anim">
                        <h1 className="font-caudex text-center text-2xl sm:text-4xl text-white drop-shadow-[0_2px_3px_rgba(0,0,0,0.8)] leading-tight font-bold p-4 px-10 sm:px-30 sm:pl-32">Your password already changed, don't forget it again!</h1>
                        <button onClick={() => setShowSuccess(false)} className="relative flex items-center justify-center h-12 min-w-[150px] md:min-w-[180px] px-6 group active:scale-95 transition-transform">
                            ...
                            <span className="relative z-10 text-white text-2xl">Okay</span>
                        </button>
                ...
                {/* Controls */}
                <div className={`absolute top-6 left-6 z-60 transition-all duration-700 ease-out ${!isZooming && !isLoggingOut && !isExiting ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-6 pointer-events-none"}`} >
                    <ButtonSidebar onClick={toggleSidebar} />
                </div>
                <div className={`absolute top-6 right-6 z-60 transition-all duration-700 ease-out ${!isZooming && !isLoggingOut && !isExiting ? "opacity-100 translate-x-0" : "opacity-0 translate-x-6 pointer-events-none"}`} >
                    <ButtonHome onClick={() => handleNavigate("/admin/home")} />
                </div>

                <AdminSidebar isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} onLogout={handleLogout} onNavigate={handleNavigate} />

                <div className="fixed inset-0 z-60 pointer-events-none bg-[#0a2a4a] transition-opacity duration-1000" style={{ opacity: isLoggingOut ? 1 : 0 }} />
                ...
                <div className={`absolute bottom-4 w-full text-center z-40 pointer-events-none transition-opacity duration-1000 delay-500 ${isZooming || isExiting || isLoggingOut ? "opacity-0" : "opacity-100"}`} >
                    <p className="text-white font-caudex text-[10px] md:text-xl tracking-widest drop-shadow-md">@Atlantis.DLOR2026. All Right Served</p>
                </div>
            </div>
        </>
    );
