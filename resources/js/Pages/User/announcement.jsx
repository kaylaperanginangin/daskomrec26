...
export default function AnnouncementPage({
    ...
    return (
        <>
            ...
            <div className="relative w-full h-screen overflow-x-hidden overflow-y-auto text-white font-caudex bg-slate-900">
                <div
                    className={`fixed top-6 left-6 z-70 transition-all duration-700 ${!inputLocked && !isLoggingOut && !isExiting ? "opacity-100" : "opacity-0 -translate-x-10 pointer-events-none"}`}
                >
                    <ButtonSidebar onClick={toggleSidebar} />
                </div>
                <div
                    className={`fixed top-6 right-6 z-70 transition-all duration-700 ${!inputLocked && !isLoggingOut && !isExiting ? "opacity-100" : "opacity-0 translate-x-10 pointer-events-none"}`}
                >
                ...
                <div className="fixed inset-0 z-10 pointer-events-none mix-blend-screen opacity-50">
                    <UnderwaterEffect />
                </div>
             ...
             {/* Contents & Footer Wrapper */}
                <div className="relative z-30 w-full flex flex-col justify-start items-center pt-[12vh]">
                    ...
                    <div
                        className={`
                        relative shrink-0 w-[95%] max-w-[420px] md:max-w-[700px] h-[600px] sm:h-[650px] md:h-[800px]
                        ${
                        ...
                        <div className="w-full h-full animate-sway-container">
                            ...
                            >
                                <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.1]">
                                    <img src={LogoDLOR} className="w-[80%] max-w-[300px] grayscale" alt="" />
                                </div>
                                ...
                                {isLive ? (
                                    <>
                                        {/* LOCKED STATE */}
                                        {!isRevealed && (
                                            <div className={`absolute inset-0 z-50 flex flex-col items-center justify-center pt-[5%] px-10 text-[#092338] ${isUnlocking ? "animate-dissolve" : ""}`}>
                                                ...
                                                <p className="text-lg font-serif text-center italic opacity-70 mb-8 max-w-[280px] sm:max-w-[320px]">The result will seal your fate as an atlantean. Are you sure you want to see it?</p>
                                                <button onClick={handleReveal} disabled={isUnlocking} className="group relative px-8 py-3 bg-[#092338] text-white rounded-full overflow-hidden shadow-lg transition-all hover:scale-105 active:scale-95">
                                                    <span className="relative z-10 font-bold tracking-widest text-sm md:text-lg flex items-center gap-2">REVEAL RESULT</span>
                                                </button>
                                            </div>
                                        )}
...
{/* If Revelaled */}
                                        {isRevealed && (
                                            <div className="w-full h-full flex flex-col items-center text-center text-[#092338] px-[10%] pt-[32%] sm:pt-[28%] md:pt-[24%] pb-[12%] animate-crt-reveal ml-1">
                                                ...
                                                {/* 2. AREA LONG TEXT (Diberi izin scroll internal dengan overflow-y-auto) */}
                                                <div className="relative z-10 flex-1 w-full overflow-y-auto hide-scrollbar my-2">
                                                    <div className="flex flex-col justify-center items-center min-h-full space-y-4">
                                                        <div className="font-serif text-[15px] sm:text-[17px] md:text-xl leading-relaxed md:leading-loose max-w-[90%] pb-4">
                                                            {userStatus === "passed" ? (
                                                                <>
                                                                    <div className="space-y-2 text-lg">
                                                                        <h1>Selamat! Kamu dinyatakan</h1>
                                                                        <div className="py-2 border-y border-dashed border-[#092338]/40 my-2">
                                                                            <span className="text-[#005f99] font-black text-4xl md:text-5xl tracking-wide block scale-110">LULUS</span>
                                                                        </div>
                                                                        <h1>seleksi tahap ini.</h1>
                                                                    </div>
                                                                    {successMessage && <div className="text-xs sm:text-sm md:text-lg opacity-70 mt-4 prose prose-sm max-w-none text-[#092338] break-words whitespace-pre-wrap" dangerouslySetInnerHTML={{ __html: successMessage }} />}
                                                                    {link && <h1 className="text-sm sm:text-sm md:text-lg italic opacity-70 mt-4"><a href={link} className="underline cursor-pointer">{link}</a></h1>}
                                                                </>
                                                            ) : userStatus === "failed" ? (
                                                                <>
                                                                    <div className="space-y-2 text-lg">
                                                                        ...
                                                                        <div className="py-2 border-y border-dashed border-[#092338]/40 my-2">
                                                                            <span className="text-red-700/80 font-black text-4xl md:text-5xl tracking-wide block">BELUM LULUS</span>
                                                                        </div>
                                                                        <h1>pada tahap ini.</h1>
                                                                    </div>
                                                                    {failMessage && <div className="text-xs sm:text-sm md:text-lg opacity-70 mt-4 prose prose-sm max-w-none text-[#092338] break-words whitespace-pre-wrap" dangerouslySetInnerHTML={{ __html: failMessage }} />}
                                                                </>
                                                            ) : (
                                                                <>
                                                                    <div className="space-y-2 text-lg">
                                                                        <h1>Bersabar ya, untuk penyeleksian masih dalam tahap</h1>
                                                                        <div className="py-2 border-y border-dashed border-[#092338]/40 my-2">
                                                                            <span className="text-orange-600/80 font-black text-4xl md:text-5xl tracking-wide block">PROSES</span>
                                                                        </div>
                                                                        <h1>pada tahap ini.</h1>
                                                                    </div>
                                                                </>
                                                ...
                                                {/* 3. TOMBOL ACTION (Dikunci di bawah menggunakan shrink-0) */}
                                                {userStatus === "passed" && shiftEnabled ? (
                                                    <div className="relative z-20 w-full h-28 md:h-40 flex justify-center items-end pb-2 shrink-0">
                                                        <button onClick={handleShiftClick} className="group relative w-64 sm:w-80 md:w-96 h-28 md:h-40 transition-all duration-300 hover:scale-105 active:scale-95">
                                                            <img src={ButtonShift} alt="Large Action Button" className="absolute inset-0 w-full h-full object-contain drop-shadow-[0_6px_8px_rgba(0,0,0,0.3)] scale-110" />
                                                            <div className="absolute inset-0 flex items-center justify-center gap-2 pt-1">
                                                                <span className="font-bold text-xl sm:text-2xl md:text-3xl text-white mb-3 drop-shadow-md group-hover:text-cyan-100 transition-colors">SHIFT</span>
                                                            </div>
                                                        </button>
                                                    </div>
                                                ) : (
                                                    <div className="relative z-20 w-full h-28 md:h-40 flex justify-center items-end pb-2 shrink-0">
                                                        <button onClick={() => handleNavigate("/User/home")} className="group relative w-64 sm:w-80 md:w-96 h-28 md:h-40 transition-all duration-300 hover:scale-105 active:scale-95">
                                                            <img src={ButtonShift} alt="Large Action Button" className="absolute inset-0 w-full h-full object-contain drop-shadow-[0_6px_8px_rgba(0,0,0,0.3)] scale-110" />
                                                            <div className="absolute inset-0 flex items-center justify-center gap-2 pt-1">
                                                                <span className="font-bold text-xl sm:text-2xl md:text-3xl text-white mb-3 drop-shadow-md group-hover:text-cyan-100 transition-colors">HOME</span>
                                                            </div>
                                                        </button>
                                                    </div>
                                                )}
                                    ...
                                    <div className={`absolute inset-0 z-50 flex flex-col items-center justify-center pt-[5%] px-10 text-[#092338]`}>
                                        <h1 className="font-bold text-xl text-center md:text-3xl tracking-[0.2em] mb-2 uppercase opacity-90 max-w-[400px]">There's nothing here yet</h1>
                                        <h1 className="text-lg font-serif text-center italic opacity-70 max-w-[280px] sm:max-w-[320px]">Check out later next time for the latest information!</h1>
                                        <button onClick={() => handleNavigate("/User/home")} className="group relative w-64 sm:w-80 md:w-96 h-28 md:h-40 transition-all duration-300 hover:scale-105 active:scale-95">
                                            <img src={ButtonShift} alt="Large Action Button" className="absolute inset-0 w-full h-full object-contain drop-shadow-[0_6px_8px_rgba(0,0,0,0.3)] scale-110" />
                                            <div className="absolute inset-0 flex items-center justify-center gap-2 pt-1">
                                                <span className="font-bold text-xl sm:text-2xl md:text-3xl text-white mb-3 drop-shadow-md group-hover:text-cyan-100 transition-colors">HOME</span>
