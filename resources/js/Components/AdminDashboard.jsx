...
const StatCard = ({ label, value, type }) => {
    ...
    return (
        ...
        >
            ...
            <div
                className={`absolute inset-0 opacity-10 pointer-events-none bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]`}
            />

            {/* Glowing Orb Background */}
            <div
                className={`absolute -right-12 -top-12 w-40 h-40 rounded-full blur-[70px] opacity-20 transition-opacity group-hover:opacity-40 ${isTotal ? "bg-cyan-500" : "bg-amber-500"}`}
            />
            ...
            <div className="flex flex-col relative z-10">
                <span
                    className={`text-xs font-serif font-bold uppercase tracking-[0.25em] mb-1 ${isTotal ? "text-cyan-200/60" : "text-amber-200/60"}`}
                >
                    {label}
                </span>
...
const Toggle = ({ isOn, onClick }) => (
    <button
        onClick={onClick}
        className={`relative w-16 h-8 transition-all duration-500 ease-out focus:outline-none group`}
    >
        {/* Track */}
        <div
            className={`absolute inset-0 rounded-sm border border-white/10 ${isOn ? "bg-emerald-900/40" : "bg-slate-900/60"}`}
        />
        ...
        >
            {/* Center Gem */}
            <div
                className={`w-2 h-2 rotate-45 ${isOn ? "bg-white shadow-[0_0_10px_white]" : "bg-slate-800"}`}
            />
        </div>
    </button>
);
...
const CoreCard = ({ core, onRequestUpdate }) => {
    ...
    const hasChanges =
        localCode !== core.code || localClue !== (core.clue || "");
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between border-b border-white/5 pb-3">
                ...
                <div
                    className={`w-2 h-2 rotate-45 transition-colors duration-500 ${core.isSolved ? "bg-cyan-400 shadow-[0_0_10px_#22d3ee]" : "bg-rose-900 border border-rose-500/50"}`}
                />
            </div>
            ...
            {/* Save Button */}
            <div
                className={`transition-all duration-500 overflow-hidden ${hasChanges ? "max-h-12 opacity-100" : "max-h-0 opacity-0"}`}
            >
                ...
                <button
                    ...
                    className="w-full py-2 bg-amber-600/20 border border-amber-500/50 hover:bg-amber-600 hover:text-black text-amber-200 text-xs font-serif font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                >
                    <svg
                        className="w-3 h-3"
                        fill="none"
...
export default function AdminDashboard({
    ...
    // Local state for toggles (derived from server data)
    const [shiftOn, setShiftOn] = useState(
        currentConfig.isi_jadwal_on ?? false,
    );
    const [announcementOn, setAnnouncementOn] = useState(
        currentConfig.pengumuman_on ?? false,
    );

    // Sync local state with server props
    useEffect(() => {
        setShiftOn(currentConfig.isi_jadwal_on ?? false);
        setAnnouncementOn(currentConfig.pengumuman_on ?? false);
    }, [currentConfig.isi_jadwal_on, currentConfig.pengumuman_on]);

    // Announcement Data (from current stage)
    const [passMessage, setPassMessage] = useState(
        currentStage?.success_message || "",
    );
    const [passLink, setPassLink] = useState(currentStage?.link || "");
    const [failMessage, setFailMessage] = useState(
        currentStage?.fail_message || "",
    );

    useEffect(() => {
        setPassMessage(currentStage?.success_message || "");
        setPassLink(currentStage?.link || "");
    ...
    // Build stage options from server data
    const stateOptions = stages.map((s) => ({ id: s.id, name: s.name }));
    ...
    const applyChange = () => {
        ...
        if (type === "Shift") {
            ...
            if (currentStageId) {
                router.put(
                    `/admin/home/configuration/${currentStageId}`,
                    {
                        isi_jadwal_on: value,
                        pengumuman_on: announcementOn,
                    },
        ...
        if (type === "announcement") {
            setAnnouncementOn(value);
            if (currentStageId) {
                router.put(
                    `/admin/home/configuration/${currentStageId}`,
                    {
                        isi_jadwal_on: shiftOn,
                        pengumuman_on: value,
                    },
                    { preserveScroll: true, preserveState: true },
                );
    ...
    const saveDrafts = () => {
        if (currentStageId) {
            router.put(
                `/admin/home/messages/${currentStageId}`,
                {
                    success_message: tempPass,
                    fail_message: tempFail,
                    link: tempPassLink,
                },
                { preserveScroll: true, preserveState: true },
            );
        ...
        setPassLink(tempPassLink);
        setFailMessage(tempFail);
        setIsEditorOpen(false);
    };

    const labelStyle =
        "text-amber-100/60 text-sm font-serif font-bold tracking-[0.25em] uppercase mb-3 block";
    ...
    return (
        <div className="relative z-50 w-full flex flex-col items-center gap-8">
            ...
            {/* CONTROL PANEL */}
            <div
                className="w-full max-w-7xl p-12 bg-[#08101a]/80 backdrop-blur-xl shadow-2xl animate-fadeIn border-y-2 border-white/5"
                        ...
                        <div className="flex items-center justify-between group">
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button
                                        onClick={openEditor}
                                        className="flex items-center gap-2 px-3 py-1.5 border border-amber-500/30 text-amber-200 hover:bg-amber-500/10 hover:border-amber-400 transition-all text-[10px] font-serif font-bold uppercase tracking-widest"
                                    >
                                        Manage
                                    </button>
                                </div>
            ...
            {/* CORES */}
            <div
                className="w-full max-w-7xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 animate-fadeIn"
            ...
            >
                {cores.map((core) => (
                    <CoreCard
                        key={core.id}
                        core={core}
                        onRequestUpdate={requestCoreUpdate}
                    />
                ))}
            </div>
                ...
                createPortal(
                    <div className="fixed inset-0 z-9999 flex items-center justify-center p-4">
                        ...
                        <div className="relative w-full max-w-lg bg-[#0a121d] border-2 border-double border-amber-600/30 p-10 shadow-[0_0_50px_rgba(217,119,6,0.15)] animate-popIn">
                            ...
                            <h3 className="text-3xl text-amber-100 font-serif font-bold mb-6 text-center tracking-wide">
                                Confirm Update
                            </h3>
                                    ...
                                    <>
                                        ...
                                        <div className="mt-6 flex flex-col gap-3">
                                            <div className="p-3 bg-black/40 border border-white/5 flex flex-col items-start">
                                                ...
                                                <span className="text-xl font-mono text-cyan-300 tracking-[0.2em]">
                                                    {
                                                        pendingAction.value
                                                            .newCode
                                                    }
                                            ...
                                            <div className="p-3 bg-black/40 border border-white/5 flex flex-col items-start">
                                                ...
                                                <span className="text-sm font-serif italic text-amber-200/80">
                                                    {
                                                        pendingAction.value
                                                            .newClue
                                                    }
                            ...
                            <div className="flex items-center justify-center gap-6 pt-8 mt-4">
                                <button
                                    onClick={cancelChange}
                                    className="px-8 py-3 border border-white/10 text-white/40 hover:text-white hover:border-white/30 transition-colors uppercase tracking-widest text-xs font-bold"
                                >
                                    Cancel
                                </button>
                                <button
                                    onClick={applyChange}
                                    className="px-10 py-3 bg-amber-700/20 border border-amber-500/50 text-amber-100 hover:bg-amber-600 hover:text-black transition-all uppercase tracking-widest text-xs font-bold shadow-[0_0_20px_rgba(217,119,6,0.2)]"
                                >
                                    Confirm
                                </button>
                            </div>
                    ...
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 lg:p-10">
                        ...
                        <div className="relative w-full max-w-7xl h-[85vh] bg-[#0a121d] border border-white/10 shadow-2xl flex flex-col overflow-hidden animate-popIn">
                            <div className="flex items-center justify-between p-6 px-8 border-b border-white/5 bg-[#0f1926]">
                                <h2 className="text-2xl font-serif text-amber-100 tracking-wider">
                                    Announcement Manager
                                </h2>
                                <div className="flex items-center bg-black/40 p-1 gap-1 border border-white/5">
                                    <button
                                        onClick={() => setIsPreviewMode(false)}
                                        className={`px-6 py-2 text-xs font-serif font-bold uppercase tracking-widest ${!isPreviewMode ? "bg-amber-700/30 text-amber-100 border border-amber-500/30" : "text-white/30"}`}
                                    >
                                        Write
                                    </button>
                                    <button
                                        onClick={() => setIsPreviewMode(true)}
                                        className={`px-6 py-2 text-xs font-serif font-bold uppercase tracking-widest ${isPreviewMode ? "bg-cyan-700/30 text-cyan-100 border border-cyan-500/30" : "text-white/30"}`}
                                    >
                                        Preview
                                    </button>
                                </div>
                                            ...
                                            <>
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex items-center bg-[#050a10]/50 border border-emerald-500/10 px-4 py-4 focus-within:border-emerald-500/40">
                                                        <span className="text-emerald-500/30 mr-4 font-serif italic">
                                                            URL
                                                        </span>
                                                        <input
                                                            type="text"
                            ...
                            <div className="p-6 border-t border-white/5 bg-[#0f1926] flex justify-end gap-6">
                                <button
                                    onClick={() => setIsEditorOpen(false)}
                                    className="text-white/30 hover:text-white text-xs font-bold uppercase tracking-widest px-4"
                                >
                                    Discard
                                </button>
                                <button
                                    onClick={saveDrafts}
                                    className="bg-amber-700/20 border border-amber-500/50 text-amber-100 px-8 py-3 hover:bg-amber-600 hover:text-black transition-all text-xs font-bold uppercase tracking-widest shadow-[0_0_20px_rgba(217,119,6,0.15)]"
                                >
                                    Save Draft
                                </button>
                            </div>
